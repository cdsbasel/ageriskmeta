...
panels <- c("DNB","GCOE_Japan","GCOE_USA","HILDA","HRS","LIKS","PHF","SAVE","SHARE_Austria","SHARE_Belgium","SHARE_Czech_Republic","SHARE_Denmark", "SHARE_Estonia", "SHARE_France", "SHARE_Germany","SHARE_Israel", "SHARE_Italy","SHARE_Netherlands","SHARE_Slovenia","SHARE_Spain","SHARE_Sweden","SHARE_Switzerland","SOEP","Usoc")
...
DNB_process_z$Gender[DNB_process_z$Gender == "1"] <- "Male"
DNB_process_z$Gender[DNB_process_z$Gender == "2"] <- "Female"
DNB_process_z$Gender <- as.factor(DNB_process_z$Gender)

ggplot(DNB_process_z, aes(x=age)) + 
...
ggplot(DNB_process_z, aes(x = age_bins,fill = Gender, group=Gender)) + 
  geom_bar(position = 'dodge') +
  geom_text(stat='count', aes(label=..count.., group=Gender ), position = position_dodge(width = 1), vjust = -1, size = 3.5) +
  #labs(title="DNB", x= "Age_bin", y= "Count, observations") +
...
ggplot(DNB_process_z_Financial) +
  geom_histogram(aes(x=Financial, y = ..density..), binwidth = 1, fill = "#A6CEE3", color = "black") + 
  scale_x_continuous(breaks=seq(0,10,by=1)) +
  stat_function(fun = dnorm, colour = "#1F78B4", args = list(mean = mean(DNB_process_z_Financial$Financial, na.rm = TRUE), sd =sd(DNB_process_z_Financial$Financial, na.rm = TRUE))) +
  labs(x= "Financial") +
  theme_minimal()
...
ggplot(DNB_process_z_Financial) +
  ...
  stat_function(fun = dnorm, colour = "#1F78B4", args = list(mean = mean(DNB_process_z_Financial$Financial_z, na.rm = TRUE), sd =sd(DNB_process_z_Financial$Financial_z, na.rm = TRUE))) +
  labs(x= "Financial_z") +
  theme_minimal()
```
...
panels <- c("DNB","GCOE_Japan","GCOE_USA","HILDA","HRS","LIKS","PHF","SAVE","SHARE_Austria","SHARE_Belgium","SHARE_Czech_Republic","SHARE_Denmark", "SHARE_Estonia", "SHARE_France", "SHARE_Germany","SHARE_Israel", "SHARE_Italy","SHARE_Netherlands","SHARE_Slovenia","SHARE_Spain","SHARE_Sweden","SHARE_Switzerland","SOEP","Usoc")   # do not include Addhealth / NLSY79_Child / NLSY79_YA panel
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 3.2, fig.height = 3.5}
#model data
...
mean_data$Panel <- factor(mean_data$Panel, levels = panels)
 ...
 ggplot() + 
   ...
   scale_shape_manual(values = rep(c(0:25), times=2), drop=FALSE, limits = levels(colordata$Panel)) + 
   stat_smooth(data = model_data, aes(x = age, y = fit, group=Panel, colour=Panel), method = "lm", formula = y ~ x, size = 0.5) +
   labs(title="Social", x= "Age", y= "Risk propensity") +
   #scale_x_continuous(breaks = c(10, 25, 40, 55, 70, 85, 100))+
   coord_cartesian(xlim = c(0,120), ylim = c(-1.5,1.5))+
   theme_bw() +
   theme(legend.position = "none") +
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender_scale <- meta_results_list_scale$meta_g.lg.gender
meta_g.lg.gender_scale_age <- meta_g.lg.gender_scale$age
summary(meta_g.lg.gender_scale_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender_scale <- meta_results_list_scale$meta_g.lg.gender
meta_g.lg.gender_scale_gender <- meta_g.lg.gender_scale$gender
summary(meta_g.lg.gender_scale_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.inter_scale <- meta_results_list_scale$meta_g.lg.inter
meta_g.lg.inter_scale_gender <- meta_g.lg.inter_scale$gender
summary(meta_g.lg.inter_scale_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.inter_scale <- meta_results_list_scale$meta_g.lg.inter
meta_g.lg.inter_scale_age_gender <- meta_g.lg.inter_scale$age_gender
summary(meta_g.lg.inter_scale_age_gender)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.quad.gender_scale <- meta_results_list_scale$meta_g.quad.gender
meta_g.quad.gender_scale_age <- meta_g.quad.gender_scale$age
summary(meta_g.quad.gender_scale_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.quad.gender_scale <- meta_results_list_scale$meta_g.quad.gender
meta_g.quad.gender_scale_age2 <- meta_g.quad.gender_scale$age2
summary(meta_g.quad.gender_scale_age2)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.quad.gender_scale <- meta_results_list_scale$meta_g.quad.gender
meta_g.quad.gender_scale_gender <- meta_g.quad.gender_scale$gender
summary(meta_g.quad.gender_scale_gender)
```
