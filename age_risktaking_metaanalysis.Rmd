---
title: "Life-course trajectories of risk-taking propensity: A coordinated analysis of longitudinal studies"
...
**Results:** Results revealed a negative relation between age and both general and domain-specific risk-taking propensity. Furthermore, females consistently reported lower levels of risk taking across the life span than males in all domains but there is little
support for the idea of an age by gender interaction. Although we found evidence of systematic and universal age differences, we also detected considerable heterogeneity across domains and samples.
...
#merge data
All_data <- Reduce(function(x, y) merge(x, y, all=TRUE), list(DHS_process_z,  GCOE_Japan_process_z, GCOE_USA_process_z, GLES_process_z, HILDA_process_z, HRS_process_z, LIKS_process_z, PHF_process_z, SAVE_process_z, SHARE_Austria_process_z, SHARE_Belgium_process_z, SHARE_Czech_Republic_process_z, SHARE_Denmark_process_z, SHARE_Estonia_process_z, SHARE_France_process_z,  SHARE_Germany_process_z, SHARE_Israel_process_z, SHARE_Italy_process_z, SHARE_Netherlands_process_z, SHARE_Slovenia_process_z, SHARE_Spain_process_z, SHARE_Sweden_process_z, SHARE_Switzerland_process_z, SOEP_process_z, UAS_process_z, USoc_process_z))
...
<br>
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 14, fig.height = 8, fig.hold="hold", out.width = "50%"}
GLES_process_z$Gender[GLES_process_z$Gender == "1"] <- "Male"
GLES_process_z$Gender[GLES_process_z$Gender == "2"] <- "Female"
GLES_process_z$Gender <- as.factor(GLES_process_z$Gender)

ggplot(GLES_process_z, aes(x=age)) + 
  geom_histogram(aes(y=..density..), binwidth = 1, colour="black", fill="white")+
  geom_density(alpha=.2, fill="#A6CEE3")+
  scale_x_continuous(breaks=seq(10,90,by=10)) +
  theme_minimal() +
...
ggplot(GLES_process_z, aes(x = age_bins,fill = Gender, group=Gender)) + 
  geom_bar(position = 'dodge') +
  geom_text(stat='count', aes(label=..count.., group=Gender ), position = position_dodge(width = 1), vjust = -1, size = 3.5) +
  #labs(title="GLES", x= "Age_bin", y= "Count, observations") +
  labs(x= "Age_bin", y= "Count, observations") +
  scale_fill_brewer(palette = "Paired") +
  theme_minimal() +
  theme(axis.text=element_text(size=18), axis.title.x = element_text(size = 24, face="bold"), axis.title.y = element_text(size = 24, face="bold"), plot.title = element_text(size=25, face="bold"))
...
<br>
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 5, fig.height = 3, fig.hold="hold", out.width = "50%"}
GLES_process_z_General <- GLES_process_z %>%
dplyr::select(ID, age, Gender, SYEAR, General, General_z) %>%
    dplyr::filter(General >= 0) %>%
    dplyr::filter(General <= 10)

ggplot(GLES_process_z_General) +
  geom_histogram(aes(x=General, y = ..density..), binwidth = 1, fill = "#A6CEE3", color = "black") + 
  scale_x_continuous(breaks=seq(0,10,by=1)) +
  stat_function(fun = dnorm, colour = "#1F78B4", args = list(mean = mean(GLES_process_z_General$General, na.rm = TRUE), sd =sd(GLES_process_z_General$General, na.rm = TRUE))) +
  labs(x= "General") +
  theme_minimal()
...
ggplot(GLES_process_z_General) +
  geom_histogram(aes(x=General_z, y = ..density..), binwidth = 0.5, fill = "#A6CEE3", color = "black") + 
  scale_x_continuous(breaks=seq(-3,3,by=1)) +
  stat_function(fun = dnorm, colour = "#1F78B4", args = list(mean = mean(GLES_process_z_General$General_z, na.rm = TRUE), sd =sd(GLES_process_z_General$General_z, na.rm = TRUE))) +
  labs(x= "General_z") +
  theme_minimal()
```
...
<br>
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 14, fig.height = 8, fig.hold="hold", out.width = "50%"}
UAS_process_z$Gender[UAS_process_z$Gender == "1"] <- "Male"
UAS_process_z$Gender[UAS_process_z$Gender == "2"] <- "Female"
UAS_process_z$Gender <- as.factor(UAS_process_z$Gender)

ggplot(UAS_process_z, aes(x=age)) + 
  geom_histogram(aes(y=..density..), binwidth = 1, colour="black", fill="white")+
  geom_density(alpha=.2, fill="#A6CEE3")+
  scale_x_continuous(breaks=seq(10,90,by=10)) +
  theme_minimal() +
...
ggplot(UAS_process_z, aes(x = age_bins,fill = Gender, group=Gender)) + 
  geom_bar(position = 'dodge') +
  geom_text(stat='count', aes(label=..count.., group=Gender ), position = position_dodge(width = 1), vjust = -1, size = 3.5) +
  #labs(title="UAS", x= "Age_bin", y= "Count, observations") +
  labs(x= "Age_bin", y= "Count, observations") +
  scale_fill_brewer(palette = "Paired") +
  theme_minimal() +
  theme(axis.text=element_text(size=18), axis.title.x = element_text(size = 24, face="bold"), axis.title.y = element_text(size = 24, face="bold"), plot.title = element_text(size=25, face="bold"))
...
<br>
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 5, fig.height = 3, fig.hold="hold", out.width = "50%"}
UAS_process_z_General <- UAS_process_z %>%
dplyr::select(ID, age, Gender, SYEAR, General, General_z) %>%
    dplyr::filter(General >= 0) %>%
    dplyr::filter(General <= 10)

ggplot(UAS_process_z_General) +
  geom_histogram(aes(x=General, y = ..density..), binwidth = 1, fill = "#A6CEE3", color = "black") + 
  scale_x_continuous(breaks=seq(0,10,by=1)) +
  stat_function(fun = dnorm, colour = "#1F78B4", args = list(mean = mean(UAS_process_z_General$General, na.rm = TRUE), sd =sd(UAS_process_z_General$General, na.rm = TRUE))) +
  labs(x= "General") +
  theme_minimal()
...
ggplot(UAS_process_z_General) +
  geom_histogram(aes(x=General_z, y = ..density..), binwidth = 0.5, fill = "#A6CEE3", color = "black") + 
  scale_x_continuous(breaks=seq(-3,3,by=1)) +
  stat_function(fun = dnorm, colour = "#1F78B4", args = list(mean = mean(UAS_process_z_General$General_z, na.rm = TRUE), sd =sd(UAS_process_z_General$General_z, na.rm = TRUE))) +
  labs(x= "General_z") +
  theme_minimal()
```
...
This section offers a detailed overview of the 7 different models included in the multilevel analysis in the paper *Life-course trajectories of risk-taking propensity: A coordinated analysis of longitudinal studies*.
...
* **Analysis:** Model <- lmer (risk ~ age + I($age^2$) + gender + (1+age|subject), data = DATA)
  ...
  ### Linear with gender and age group model
...
### Linear with gender and age group interaction model
...
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2")
...
rownames(results_g.fixed)[rownames(results_g.fixed)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_g.fixed <- results_g.fixed[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "N.group", "N.obs", "LL"),]
...
rownames(results_g.lg)[rownames(results_g.lg)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_g.lg <- results_g.lg[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 10.5, fig.cap = "Figure: Age trajectories of general risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of linear model results."}

panels <- c("DHS", "GCOE_Japan", "GCOE_USA", "GLES", "HILDA", "HRS", "LIKS", "PHF", "SAVE",
            "SHARE_Austria", "SHARE_Belgium", "SHARE_Czech_Republic", "SHARE_Denmark", "SHARE_Estonia", "SHARE_France", "SHARE_Germany","SHARE_Israel", "SHARE_Italy", "SHARE_Netherlands", "SHARE_Slovenia", "SHARE_Spain", "SHARE_Sweden", "SHARE_Switzerland", "SOEP","UAS","USoc")   
...
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2")
...
rownames(results_g.lg.gender)[rownames(results_g.lg.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "d.c.age:gender_Std.Error2")
...
rownames(results_g.lg.inter)[rownames(results_g.lg.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_g.lg.inter)[rownames(results_g.lg.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_g.lg.inter)[rownames(results_g.lg.inter)=="d.c.age:gender_Std.Error1"] <- "d.c.age:gender_Std.Error"
...
results_g.lg.inter <- results_g.lg.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "I(d.c.age^2)_Std.Error2")
...
rownames(results_g.quad)[rownames(results_g.quad)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_g.quad)[rownames(results_g.quad)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
...
results_g.quad <- results_g.quad[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "I(d.c.age^2)_Estimate", "I(d.c.age^2)_Std.Error", "I(d.c.age^2)_t.value", "I(d.c.age^2)_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 7, fig.cap = "Figure: Age trajectories of general risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of quadratic model results."}
#model data
...
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "I(d.c.age^2)_Std.Error2","gender_Std.Error2")
...
rownames(results_g.quad.gender)[rownames(results_g.quad.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_g.quad.gender)[rownames(results_g.quad.gender)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
...
results_g.quad.gender <- results_g.quad.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "I(d.c.age^2)_Estimate", "I(d.c.age^2)_Std.Error", "I(d.c.age^2)_t.value", "I(d.c.age^2)_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_f.fixed)[rownames(results_f.fixed)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_f.fixed <- results_f.fixed[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "N.group", "N.obs", "LL"),]
...
rownames(results_f.lg)[rownames(results_f.lg)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_f.lg <- results_f.lg[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 13.333, fig.height = 24.5, fig.cap = "Figure: Age trajectories of financial risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of linear model results."}
#model data
...
rownames(results_f.lg.gender)[rownames(results_f.lg.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_f.lg.gender <- results_f.lg.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_f.lg.inter)[rownames(results_f.lg.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_f.lg.inter)[rownames(results_f.lg.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_f.lg.inter)[rownames(results_f.lg.inter)=="d.c.age:gender_Std.Error1"] <- "d.c.age:gender_Std.Error"
...
results_f.lg.inter <- results_f.lg.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_f.quad)[rownames(results_f.quad)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_f.quad)[rownames(results_f.quad)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 13.333, fig.height = 14, fig.cap = "Figure: Age trajectories of financial risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of quadratic model results."}
#model data
...
rownames(results_f.quad.gender)[rownames(results_f.quad.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_f.quad.gender)[rownames(results_f.quad.gender)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
...
results_f.quad.gender <- results_f.quad.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "I(d.c.age^2)_Estimate", "I(d.c.age^2)_Std.Error", "I(d.c.age^2)_t.value", "I(d.c.age^2)_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 13.333, fig.height = 14, fig.cap = "Figure: Age trajectories of financial risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. Circle = female, triangle = male. The lines represent the age trajectory of quadratic with gender model results. Solid line = female, dotted line = male."}
#model data
...
rownames(results_d.fixed)[rownames(results_d.fixed)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_d.fixed <- results_d.fixed[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "N.group", "N.obs", "LL"),]
...
rownames(results_d.lg)[rownames(results_d.lg)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_d.lg <- results_d.lg[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_d.lg.gender)[rownames(results_d.lg.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_d.lg.gender <- results_d.lg.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_d.lg.inter)[rownames(results_d.lg.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_d.lg.inter)[rownames(results_d.lg.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_d.lg.inter)[rownames(results_d.lg.inter)=="d.c.age:gender_Std.Error1"] <- "d.c.age:gender_Std.Error"
...
results_d.lg.inter <- results_d.lg.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_d.quad)[rownames(results_d.quad)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_d.quad)[rownames(results_d.quad)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
results_d.quad <- results_d.quad[, colSums(is.na(results_d.quad)) != nrow(results_d.quad)]
results_d.quad <- results_d.quad[complete.cases(results_d.quad), ]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 6.66, fig.height = 3.5, fig.cap = "Figure: Age trajectories of driving risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of quadratic model results."}
#model data
...
rownames(results_d.quad.gender)[rownames(results_d.quad.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_d.quad.gender)[rownames(results_d.quad.gender)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
rownames(results_d.quad.gender)[rownames(results_d.quad.gender)=="gender_Std.Error1"] <- "gender_Std.Error1"
results_d.quad.gender <- results_d.quad.gender[, colSums(is.na(results_d.quad.gender)) != nrow(results_d.quad.gender)]
results_d.quad.gender <- results_d.quad.gender[complete.cases(results_d.quad.gender), ]
...
results_d.quad.gender <- results_d.quad.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "I(d.c.age^2)_Estimate", "I(d.c.age^2)_Std.Error", "I(d.c.age^2)_t.value", "I(d.c.age^2)_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"), ]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 6.66, fig.height = 3.5,, fig.cap = "Figure: Age trajectories of driving risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. Circle = female, triangle = male. The lines represent the age trajectory of quadratic with gender model results. Solid line = female, dotted line = male."}
#model data
...
rownames(results_r.fixed)[rownames(results_r.fixed)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_r.fixed <- results_r.fixed[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "N.group", "N.obs", "LL"),]
...
rownames(results_r.lg)[rownames(results_r.lg)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_r.lg <- results_r.lg[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 3.5, fig.cap = "Figure: Age trajectories of recreational risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of linear model results."}
#model data
...
rownames(results_r.lg.gender)[rownames(results_r.lg.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_r.lg.gender <- results_r.lg.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_r.lg.inter)[rownames(results_r.lg.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_r.lg.inter)[rownames(results_r.lg.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_r.lg.inter)[rownames(results_r.lg.inter)=="d.c.age:gender_Std.Error1"] <- "d.c.age:gender_Std.Error"
...
results_r.lg.inter <- results_r.lg.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_r.quad)[rownames(results_r.quad)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_r.quad)[rownames(results_r.quad)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
results_r.quad <- results_r.quad[, colSums(is.na(results_r.quad)) != nrow(results_r.quad)]
results_r.quad <- results_r.quad[complete.cases(results_r.quad), ]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 6.66, fig.height = 3.5, fig.cap = "Figure: Age trajectories of recreational risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of quadratic model results."}
#model data
...
rownames(results_r.quad.gender)[rownames(results_r.quad.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_r.quad.gender)[rownames(results_r.quad.gender)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
rownames(results_r.quad.gender)[rownames(results_r.quad.gender)=="gender_Std.Error1"] <- "gender_Std.Error1"
results_r.quad.gender <- results_r.quad.gender[, colSums(is.na(results_r.quad.gender)) != nrow(results_r.quad.gender)]
results_r.quad.gender <- results_r.quad.gender[complete.cases(results_r.quad.gender),]
...
results_r.quad.gender <- results_r.quad.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "I(d.c.age^2)_Estimate", "I(d.c.age^2)_Std.Error", "I(d.c.age^2)_t.value", "I(d.c.age^2)_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_o.fixed)[rownames(results_o.fixed)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_o.fixed <- results_o.fixed[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "N.group", "N.obs", "LL"),]
...
rownames(results_o.lg)[rownames(results_o.lg)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_o.lg <- results_o.lg[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 3.5, fig.cap = "Figure: Age trajectories of occupational risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of linear model results."}
#model data
...
rownames(results_o.lg.gender)[rownames(results_o.lg.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_o.lg.gender <- results_o.lg.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_o.lg.inter)[rownames(results_o.lg.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_o.lg.inter)[rownames(results_o.lg.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_o.lg.inter)[rownames(results_o.lg.inter)=="d.c.age:gender_Std.Error1"] <- "d.c.age:gender_Std.Error"
...
results_o.lg.inter <- results_o.lg.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_o.quad)[rownames(results_o.quad)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_o.quad)[rownames(results_o.quad)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
results_o.quad <- results_o.quad[, colSums(is.na(results_o.quad)) != nrow(results_o.quad)]
results_o.quad <- results_o.quad[complete.cases(results_o.quad), ]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 6.66, fig.height = 3.5, fig.cap = "Figure: Age trajectories of occupational risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of quadratic model results."}
#model data
...
rownames(results_o.quad.gender)[rownames(results_o.quad.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_o.quad.gender)[rownames(results_o.quad.gender)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
rownames(results_o.quad.gender)[rownames(results_o.quad.gender)=="gender_Std.Error1"] <- "gender_Std.Error1"
results_o.quad.gender <- results_o.quad.gender[, colSums(is.na(results_o.quad.gender)) != nrow(results_o.quad.gender)]
results_o.quad.gender <- results_o.quad.gender[complete.cases(results_o.quad.gender),]
...
results_o.quad.gender <- results_o.quad.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "I(d.c.age^2)_Estimate", "I(d.c.age^2)_Std.Error", "I(d.c.age^2)_t.value", "I(d.c.age^2)_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"), ]
...
rownames(results_h.fixed)[rownames(results_h.fixed)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_h.fixed <- results_h.fixed[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "N.group", "N.obs", "LL"),]
...
rownames(results_h.lg)[rownames(results_h.lg)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_h.lg <- results_h.lg[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 3.5, fig.cap = "Figure: Age trajectories of health risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of linear model results."}
#model data
...
rownames(results_h.lg.gender)[rownames(results_h.lg.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_h.lg.gender <- results_h.lg.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_h.lg.inter)[rownames(results_h.lg.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_h.lg.inter)[rownames(results_h.lg.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_h.lg.inter)[rownames(results_h.lg.inter)=="d.c.age:gender_Std.Error1"] <- "d.c.age:gender_Std.Error"
...
results_h.lg.inter <- results_h.lg.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]
...
rownames(results_h.quad)[rownames(results_h.quad)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_h.quad)[rownames(results_h.quad)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
results_h.quad <- results_h.quad[, colSums(is.na(results_h.quad)) != nrow(results_h.quad)]
results_h.quad <- results_h.quad[complete.cases(results_h.quad), ]
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.width = 6.66, fig.height = 3.5, fig.cap = "Figure: Age trajectories of health risk-taking propensity for each sample. The points represent moving average (window size = 5, window step = 2) of raw data. The lines represent the age trajectory of quadratic model results."}
#model data
...
rownames(results_h.quad.gender)[rownames(results_h.quad.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_h.quad.gender)[rownames(results_h.quad.gender)=="I(d.c.age^2)_Std.Error1"] <- "I(d.c.age^2)_Std.Error"
rownames(results_h.quad.gender)[rownames(results_h.quad.gender)=="gender_Std.Error1"] <- "gender_Std.Error1"
results_h.quad.gender <- results_h.quad.gender[, colSums(is.na(results_h.quad.gender)) != nrow(results_h.quad.gender)]
results_h.quad.gender <- results_h.quad.gender[complete.cases(results_h.quad.gender), ]
...
results_h.quad.gender <- results_h.quad.gender[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "I(d.c.age^2)_Estimate", "I(d.c.age^2)_Std.Error", "I(d.c.age^2)_t.value", "I(d.c.age^2)_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"), ]
...
rownames(results_s.fixed)[rownames(results_s.fixed)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_s.fixed <- results_s.fixed[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "N.group", "N.obs", "LL"), 1, drop = FALSE]
...
rownames(results_s.lg)[rownames(results_s.lg)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
results_s.lg <- results_s.lg[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"), 1, drop = FALSE]
...
rownames(results_s.lg.gender)[rownames(results_s.lg.gender)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
...
rownames(results_s.lg.inter)[rownames(results_s.lg.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_s.lg.inter)[rownames(results_s.lg.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_s.lg.inter)[rownames(results_s.lg.inter)=="d.c.age:gender_Std.Error1"] <- "d.c.age:gender_Std.Error"
...
results_s.lg.inter <- results_s.lg.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate", "gender_Std.Error", "gender_t.value", "gender_p.value", "d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"), 1, drop = FALSE]
...
**Meta-analysis with four moderators (continent, mean age, scale range, survey year, survey year):**
<br>
*ICC's results*
```{r, echo = F, warning = FALSE, message = FALSE}
setwd(".../data/results_data")
...
**Meta-analysis with four moderators (continent, mean age, scale range, survey year):**
<br>
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.quad_all <- meta_results_list_all$meta_g.quad
meta_g.quad_all_age <- meta_g.quad_all$age
summary(meta_g.quad_all_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.quad_all <- meta_results_list_all$meta_g.quad
meta_g.quad_all_age2 <- meta_g.quad_all$age2
summary(meta_g.quad_all_age2)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.quad.gender_all <- meta_results_list_all$meta_g.quad.gender
meta_g.quad.gender_all_age <- meta_g.quad.gender_all$age
summary(meta_g.quad.gender_all_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.quad.gender_all <- meta_results_list_all$meta_g.quad.gender
meta_g.quad.gender_all_age2 <- meta_g.quad.gender_all$age2
summary(meta_g.quad.gender_all_age2)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.quad.gender_all <- meta_results_list_all$meta_g.quad.gender
meta_g.quad.gender_all_gender <- meta_g.quad.gender_all$gender
summary(meta_g.quad.gender_all_gender)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.quad <- meta_results_list$meta_d.quad
meta_d.quad_age <- meta_d.quad$age
summary(meta_d.quad_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.quad <- meta_results_list$meta_d.quad
meta_d.quad_age2 <- meta_d.quad$age2
summary(meta_d.quad_age2)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.quad.gender <- meta_results_list$meta_d.quad.gender
meta_d.quad.gender_age <- meta_d.quad.gender$age
summary(meta_d.quad.gender_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.quad.gender <- meta_results_list$meta_d.quad.gender
meta_d.quad.gender_age2 <- meta_d.quad.gender$age2
summary(meta_d.quad.gender_age2)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.quad.gender <- meta_results_list$meta_d.quad.gender
meta_d.quad.gender_gender <- meta_d.quad.gender$gender
summary(meta_d.quad.gender_gender)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.quad <- meta_results_list$meta_r.quad
meta_r.quad_age <- meta_r.quad$age
summary(meta_r.quad_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.quad <- meta_results_list$meta_r.quad
meta_r.quad_age2 <- meta_r.quad$age2
summary(meta_r.quad_age2)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.quad.gender <- meta_results_list$meta_r.quad.gender
meta_r.quad.gender_age <- meta_r.quad.gender$age
summary(meta_r.quad.gender_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.quad.gender <- meta_results_list$meta_r.quad.gender
meta_r.quad.gender_age2 <- meta_r.quad.gender$age2
summary(meta_r.quad.gender_age2)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.quad.gender <- meta_results_list$meta_r.quad.gender
meta_r.quad.gender_gender <- meta_r.quad.gender$gender
summary(meta_r.quad.gender_gender)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.quad <- meta_results_list$meta_o.quad
meta_o.quad_age <- meta_o.quad$age
summary(meta_o.quad_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.quad <- meta_results_list$meta_o.quad
meta_o.quad_age2 <- meta_o.quad$age2
summary(meta_o.quad_age2)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.quad.gender <- meta_results_list$meta_o.quad.gender
meta_o.quad.gender_age <- meta_o.quad.gender$age
summary(meta_o.quad.gender_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.quad.gender <- meta_results_list$meta_o.quad.gender
meta_o.quad.gender_age2 <- meta_o.quad.gender$age2
summary(meta_o.quad.gender_age2)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.quad.gender <- meta_results_list$meta_o.quad.gender
meta_o.quad.gender_gender <- meta_o.quad.gender$gender
summary(meta_o.quad.gender_gender)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.quad <- meta_results_list$meta_h.quad
meta_h.quad_age <- meta_h.quad$age
summary(meta_h.quad_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.quad <- meta_results_list$meta_h.quad
meta_h.quad_age2 <- meta_h.quad$age2
summary(meta_h.quad_age2)
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.quad.gender <- meta_results_list$meta_h.quad.gender
meta_h.quad.gender_age <- meta_h.quad.gender$age
summary(meta_h.quad.gender_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.quad.gender <- meta_results_list$meta_h.quad.gender
meta_h.quad.gender_age2 <- meta_h.quad.gender$age2
summary(meta_h.quad.gender_age2)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.quad.gender <- meta_results_list$meta_h.quad.gender
meta_h.quad.gender_gender <- meta_h.quad.gender$gender
summary(meta_h.quad.gender_gender)
```
...
### 7.1. Samples including 3 or more waves
```{r, echo = F, warning = FALSE, message = FALSE}
Total.Number <- c(9, 20, 3, 3, 3, 3, 1)
Lg <- c(1, 0, 0, 0, 0, 0, 0)
Lg.gender <- c(7, 17, 2, 1, 3, 1, 1)
...
sketch = htmltools::withTags(table(
  ...
  thead(
    tr(
      ...
      th(colspan = 3, 'Number of samples with best-fitting'),
    ),
    tr(
...
### 7.2. Samples including 4 or more waves
```{r, echo = F, warning = FALSE, message = FALSE}
Total.Number <- c(7, 16, 2,  2, 2, 2, 0)
Lg <- c(0, 0, 0, 0, 0, 0, 0)
Lg.gender <- c(5, 11, 1, 1, 1, 1, 0)
...
sketch = htmltools::withTags(table(
  ...
  thead(
    tr(
      th(rowspan = 2, ''),
      th(rowspan = 2, 'Total number'),
      th(colspan = 5, 'Number of samples with best-fitting'),
    ),
    tr(
...
### 8.1. Multilevel model results and meta analysis results{.tabset}

#### General risk-taking {.tabset}
...
results_g.lg.gender.group <- results_table_list_cohort$results_g.lg.gender.group
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2")
results_g.lg.gender.group <- results_g.lg.gender.group[!(row.names(results_g.lg.gender.group) %in% remove),]
rownames(results_g.lg.gender.group)[rownames(results_g.lg.gender.group)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_g.lg.gender.group)[rownames(results_g.lg.gender.group)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_g.lg.gender.group)[rownames(results_g.lg.gender.group)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_g.lg.gender.group)[rownames(results_g.lg.gender.group)=="age.group_Std.Error1"] <- "age.group_Std.Error"
results_g.lg.gender.group <- results_g.lg.gender.group[, colSums(is.na(results_g.lg.gender.group)) != nrow(results_g.lg.gender.group)]
results_g.lg.gender.group <- results_g.lg.gender.group[complete.cases(results_g.lg.gender.group), ]

results_g.lg.gender.group[c(1:12,17:21,24), ] <- purrr::modify_if(results_g.lg.gender.group[c(1:12,17:21,24), ], ~is.numeric(.), ~round(., 2))
results_g.lg.gender.group[c(22:23), ] <- purrr::modify_if(results_g.lg.gender.group[c(22:23), ], ~is.numeric(.), ~round(., 0))

results_g.lg.gender.group[13,] <- p_judge(results_g.lg.gender.group[13,])
results_g.lg.gender.group[14,] <- p_judge(results_g.lg.gender.group[14,])
results_g.lg.gender.group[15,] <- p_judge(results_g.lg.gender.group[15,])
results_g.lg.gender.group[16,] <- p_judge(results_g.lg.gender.group[16,])

results_g.lg.gender.group <- results_g.lg.gender.group[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_g.lg.gender.group, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_g.lg.gender.group))), pageLength = nrow(results_g.lg.gender.group)))
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
setwd(".../data/results_data")
meta_results_list_cohort <- readRDS(file = "meta_results_list_cohort.Rds")

meta_g.lg.gender.group <- meta_results_list_cohort$meta_g.lg.gender.group
meta_g.lg.gender.group_age <- meta_g.lg.gender.group$age
summary(meta_g.lg.gender.group_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender.group <- meta_results_list_cohort$meta_g.lg.gender.group
meta_g.lg.gender.group_gender <- meta_g.lg.gender.group$gender
summary(meta_g.lg.gender.group_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender.group <- meta_results_list_cohort$meta_g.lg.gender.group
meta_g.lg.gender.group_age.group <- meta_g.lg.gender.group$age.group
summary(meta_g.lg.gender.group_age.group)
```
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Age trajectories of general risk-taking propensity for each sample and meta-analysis. The solid black line indicates the average trajectory weighted by the sample size. The colored lines represent the trajectory for individual samples"}
setwd(".../data/results_data")
plot_model_data_list_cohort <- readRDS(file = "plot_model_data_list_cohort.Rds") 
...
###function: create a empty figure_list for saving all plot
index=0
plot_name_list <- list()  
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_g.lg.gender.group.inter <- results_table_list_cohort$results_g.lg.gender.group.inter
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2", "d.c.age:gender_Std.Error2", "d.c.age:age.group_Std.Error2", "gender:age.group_Std.Error2", "d.c.age:gender:age.group_Std.Error2")
results_g.lg.gender.group.inter <- results_g.lg.gender.group.inter[!(row.names(results_g.lg.gender.group.inter) %in% remove),]
rownames(results_g.lg.gender.group.inter)[rownames(results_g.lg.gender.group.inter)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_g.lg.gender.group.inter)[rownames(results_g.lg.gender.group.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_g.lg.gender.group.inter)[rownames(results_g.lg.gender.group.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_g.lg.gender.group.inter)[rownames(results_g.lg.gender.group.inter)=="age.group_Std.Error1"] <- "age.group_Std.Error"
rownames(results_g.lg.gender.group.inter)[rownames(results_g.lg.gender.group.inter)=="d.c.age:gender_group_Std.Error1"] <- "d.c.age:gender_Std.Error"
rownames(results_g.lg.gender.group.inter)[rownames(results_g.lg.gender.group.inter)=="d.c.age:age.group_Std.Error1"] <- "d.c.age:age.group_Std.Error"
rownames(results_g.lg.gender.group.inter)[rownames(results_g.lg.gender.group.inter)=="gender:age.group_Std.Error1"] <- "gender:age.group_Std.Error"
rownames(results_g.lg.gender.group.inter)[rownames(results_g.lg.gender.group.inter)=="d.c.age:gender:age.group_Std.Error1"] <- "d.c.age:gender:age.group_Std.Error"
results_g.lg.gender.group.inter <- results_g.lg.gender.group.inter[, colSums(is.na(results_g.lg.gender.group.inter)) != nrow(results_g.lg.gender.group.inter)]
results_g.lg.gender.group.inter <- results_g.lg.gender.group.inter[complete.cases(results_g.lg.gender.group.inter), ]

results_g.lg.gender.group.inter[c(1:24,33:37,40), ] <- purrr::modify_if(results_g.lg.gender.group.inter[c(1:24,33:37,40), ], ~is.numeric(.), ~round(., 2))
results_g.lg.gender.group.inter[c(38:39), ] <- purrr::modify_if(results_g.lg.gender.group.inter[c(38:39), ], ~is.numeric(.), ~round(., 0))

results_g.lg.gender.group.inter[25,] <- p_judge(results_g.lg.gender.group.inter[25,])
results_g.lg.gender.group.inter[26,] <- p_judge(results_g.lg.gender.group.inter[26,])
results_g.lg.gender.group.inter[27,] <- p_judge(results_g.lg.gender.group.inter[27,])
results_g.lg.gender.group.inter[28,] <- p_judge(results_g.lg.gender.group.inter[28,])
results_g.lg.gender.group.inter[29,] <- p_judge(results_g.lg.gender.group.inter[29,])
results_g.lg.gender.group.inter[30,] <- p_judge(results_g.lg.gender.group.inter[30,])
results_g.lg.gender.group.inter[31,] <- p_judge(results_g.lg.gender.group.inter[31,])
results_g.lg.gender.group.inter[32,] <- p_judge(results_g.lg.gender.group.inter[32,])

results_g.lg.gender.group.inter <- results_g.lg.gender.group.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value","d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "d.c.age:age.group_Estimate", "d.c.age:age.group_Std.Error", "d.c.age:age.group_t.value", "d.c.age:age.group_p.value", "gender:age.group_Estimate", "gender:age.group_Std.Error", "gender:age.group_t.value", "gender:age.group_p.value", "d.c.age:gender:age.group_Estimate", "d.c.age:gender:age.group_Std.Error", "d.c.age:gender:age.group_t.value", "d.c.age:gender:age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_g.lg.gender.group.inter, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_g.lg.gender.group.inter))), pageLength = nrow(results_g.lg.gender.group.inter)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender.group.inter <- meta_results_list_cohort$meta_g.lg.gender.group.inter
meta_g.lg.gender.group.inter_age <- meta_g.lg.gender.group.inter$age
summary(meta_g.lg.gender.group.inter_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender.group.inter <- meta_results_list_cohort$meta_g.lg.gender.group.inter
meta_g.lg.gender.group.inter_gender <- meta_g.lg.gender.group.inter$gender
summary(meta_g.lg.gender.group.inter_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender.group.inter <- meta_results_list_cohort$meta_g.lg.gender.group.inter
meta_g.lg.gender.group.inter_age.group <- meta_g.lg.gender.group.inter$age.group
summary(meta_g.lg.gender.group.inter_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender.group.inter <- meta_results_list_cohort$meta_g.lg.gender.group.inter
meta_g.lg.gender.group.inter_age_gender <- meta_g.lg.gender.group.inter$age_gender
summary(meta_g.lg.gender.group.inter_age_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender.group.inter <- meta_results_list_cohort$meta_g.lg.gender.group.inter
meta_g.lg.gender.group.inter_age_age.group <- meta_g.lg.gender.group.inter$age_age.group
summary(meta_g.lg.gender.group.inter_age_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender.group.inter <- meta_results_list_cohort$meta_g.lg.gender.group.inter
meta_g.lg.gender.group.inter_gender_age.group <- meta_g.lg.gender.group.inter$gender_age.group
summary(meta_g.lg.gender.group.inter_gender_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_g.lg.gender.group.inter <- meta_results_list_cohort$meta_g.lg.gender.group.inter
meta_g.lg.gender.group.inter_age_gender_age.group <- meta_g.lg.gender.group.inter$age_gender_age.group
summary(meta_g.lg.gender.group.inter_age_gender_age.group)
```
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_f.lg.gender.group <- results_table_list_cohort$results_f.lg.gender.group
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2")
results_f.lg.gender.group <- results_f.lg.gender.group[!(row.names(results_f.lg.gender.group) %in% remove),]
rownames(results_f.lg.gender.group)[rownames(results_f.lg.gender.group)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_f.lg.gender.group)[rownames(results_f.lg.gender.group)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_f.lg.gender.group)[rownames(results_f.lg.gender.group)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_f.lg.gender.group)[rownames(results_f.lg.gender.group)=="age.group_Std.Error1"] <- "age.group_Std.Error"
results_f.lg.gender.group <- results_f.lg.gender.group[, colSums(is.na(results_f.lg.gender.group)) != nrow(results_f.lg.gender.group)]
results_f.lg.gender.group <- results_f.lg.gender.group[complete.cases(results_f.lg.gender.group), ]

results_f.lg.gender.group[c(1:12,17:21,24), ] <- purrr::modify_if(results_f.lg.gender.group[c(1:12,17:21,24), ], ~is.numeric(.), ~round(., 2))
results_f.lg.gender.group[c(22:23), ] <- purrr::modify_if(results_f.lg.gender.group[c(22:23), ], ~is.numeric(.), ~round(., 0))

results_f.lg.gender.group[13,] <- p_judge(results_f.lg.gender.group[13,])
results_f.lg.gender.group[14,] <- p_judge(results_f.lg.gender.group[14,])
results_f.lg.gender.group[15,] <- p_judge(results_f.lg.gender.group[15,])
results_f.lg.gender.group[16,] <- p_judge(results_f.lg.gender.group[16,])

results_f.lg.gender.group <- results_f.lg.gender.group[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_f.lg.gender.group, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_f.lg.gender.group))), pageLength = nrow(results_f.lg.gender.group)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group <- meta_results_list_cohort$meta_f.lg.gender.group
meta_f.lg.gender.group_age <- meta_f.lg.gender.group$age
summary(meta_f.lg.gender.group_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group <- meta_results_list_cohort$meta_f.lg.gender.group
meta_f.lg.gender.group_gender <- meta_f.lg.gender.group$gender
summary(meta_f.lg.gender.group_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group <- meta_results_list_cohort$meta_f.lg.gender.group
meta_f.lg.gender.group_age.group <- meta_f.lg.gender.group$age.group
summary(meta_f.lg.gender.group_age.group)
```
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Age trajectories of financial risk-taking propensity for each sample and meta-analysis. The solid black line indicates the average trajectory weighted by the sample size. The colored lines represent the trajectory for individual samples"}
Fianacial <- figure_list$plot_f.lg.gender.group
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_f.lg.gender.group.inter <- results_table_list_cohort$results_f.lg.gender.group.inter
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2", "d.c.age:gender_Std.Error2", "d.c.age:age.group_Std.Error2", "gender:age.group_Std.Error2", "d.c.age:gender:age.group_Std.Error2")
results_f.lg.gender.group.inter <- results_f.lg.gender.group.inter[!(row.names(results_f.lg.gender.group.inter) %in% remove),]
rownames(results_f.lg.gender.group.inter)[rownames(results_f.lg.gender.group.inter)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_f.lg.gender.group.inter)[rownames(results_f.lg.gender.group.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_f.lg.gender.group.inter)[rownames(results_f.lg.gender.group.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_f.lg.gender.group.inter)[rownames(results_f.lg.gender.group.inter)=="age.group_Std.Error1"] <- "age.group_Std.Error"
rownames(results_f.lg.gender.group.inter)[rownames(results_f.lg.gender.group.inter)=="d.c.age:gender_group_Std.Error1"] <- "d.c.age:gender_Std.Error"
rownames(results_f.lg.gender.group.inter)[rownames(results_f.lg.gender.group.inter)=="d.c.age:age.group_Std.Error1"] <- "d.c.age:age.group_Std.Error"
rownames(results_f.lg.gender.group.inter)[rownames(results_f.lg.gender.group.inter)=="gender:age.group_Std.Error1"] <- "gender:age.group_Std.Error"
rownames(results_f.lg.gender.group.inter)[rownames(results_f.lg.gender.group.inter)=="d.c.age:gender:age.group_Std.Error1"] <- "d.c.age:gender:age.group_Std.Error"
results_f.lg.gender.group.inter <- results_f.lg.gender.group.inter[, colSums(is.na(results_f.lg.gender.group.inter)) != nrow(results_f.lg.gender.group.inter)]
results_f.lg.gender.group.inter <- results_f.lg.gender.group.inter[complete.cases(results_f.lg.gender.group.inter), ]

results_f.lg.gender.group.inter[c(1:24,33:37,40), ] <- purrr::modify_if(results_f.lg.gender.group.inter[c(1:24,33:37,40), ], ~is.numeric(.), ~round(., 2))
results_f.lg.gender.group.inter[c(38:39), ] <- purrr::modify_if(results_f.lg.gender.group.inter[c(38:39), ], ~is.numeric(.), ~round(., 0))

results_f.lg.gender.group.inter[25,] <- p_judge(results_f.lg.gender.group.inter[25,])
results_f.lg.gender.group.inter[26,] <- p_judge(results_f.lg.gender.group.inter[26,])
results_f.lg.gender.group.inter[27,] <- p_judge(results_f.lg.gender.group.inter[27,])
results_f.lg.gender.group.inter[28,] <- p_judge(results_f.lg.gender.group.inter[28,])
results_f.lg.gender.group.inter[29,] <- p_judge(results_f.lg.gender.group.inter[29,])
results_f.lg.gender.group.inter[30,] <- p_judge(results_f.lg.gender.group.inter[30,])
results_f.lg.gender.group.inter[31,] <- p_judge(results_f.lg.gender.group.inter[31,])
results_f.lg.gender.group.inter[32,] <- p_judge(results_f.lg.gender.group.inter[32,])

results_f.lg.gender.group.inter <- results_f.lg.gender.group.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value","d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "d.c.age:age.group_Estimate", "d.c.age:age.group_Std.Error", "d.c.age:age.group_t.value", "d.c.age:age.group_p.value", "gender:age.group_Estimate", "gender:age.group_Std.Error", "gender:age.group_t.value", "gender:age.group_p.value", "d.c.age:gender:age.group_Estimate", "d.c.age:gender:age.group_Std.Error", "d.c.age:gender:age.group_t.value", "d.c.age:gender:age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_f.lg.gender.group.inter, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_f.lg.gender.group.inter))), pageLength = nrow(results_f.lg.gender.group.inter)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group.inter <- meta_results_list_cohort$meta_f.lg.gender.group.inter
meta_f.lg.gender.group.inter_age <- meta_f.lg.gender.group.inter$age
summary(meta_f.lg.gender.group.inter_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group.inter <- meta_results_list_cohort$meta_f.lg.gender.group.inter
meta_f.lg.gender.group.inter_gender <- meta_f.lg.gender.group.inter$gender
summary(meta_f.lg.gender.group.inter_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group.inter <- meta_results_list_cohort$meta_f.lg.gender.group.inter
meta_f.lg.gender.group.inter_age.group <- meta_f.lg.gender.group.inter$age.group
summary(meta_f.lg.gender.group.inter_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group.inter <- meta_results_list_cohort$meta_f.lg.gender.group.inter
meta_f.lg.gender.group.inter_age_gender <- meta_f.lg.gender.group.inter$age_gender
summary(meta_f.lg.gender.group.inter_age_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group.inter <- meta_results_list_cohort$meta_f.lg.gender.group.inter
meta_f.lg.gender.group.inter_age_age.group <- meta_f.lg.gender.group.inter$age_age.group
summary(meta_f.lg.gender.group.inter_age_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group.inter <- meta_results_list_cohort$meta_f.lg.gender.group.inter
meta_f.lg.gender.group.inter_gender_age.group <- meta_f.lg.gender.group.inter$gender_age.group
summary(meta_f.lg.gender.group.inter_gender_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_f.lg.gender.group.inter <- meta_results_list_cohort$meta_f.lg.gender.group.inter
meta_f.lg.gender.group.inter_age_gender_age.group <- meta_f.lg.gender.group.inter$age_gender_age.group
summary(meta_f.lg.gender.group.inter_age_gender_age.group)
```
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Age trajectories of financial risk-taking propensity for each sample and meta-analysis. The solid black line indicates the average trajectory weighted by the sample size. The colored lines represent the trajectory for individual samples"}
Fianacial <- figure_list$plot_f.lg.gender.group.inter
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_d.lg.gender.group <- results_table_list_cohort$results_d.lg.gender.group
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2")
results_d.lg.gender.group <- results_d.lg.gender.group[!(row.names(results_d.lg.gender.group) %in% remove),]
rownames(results_d.lg.gender.group)[rownames(results_d.lg.gender.group)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_d.lg.gender.group)[rownames(results_d.lg.gender.group)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_d.lg.gender.group)[rownames(results_d.lg.gender.group)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_d.lg.gender.group)[rownames(results_d.lg.gender.group)=="age.group_Std.Error1"] <- "age.group_Std.Error"
results_d.lg.gender.group <- results_d.lg.gender.group[, colSums(is.na(results_d.lg.gender.group)) != nrow(results_d.lg.gender.group)]
results_d.lg.gender.group <- results_d.lg.gender.group[complete.cases(results_d.lg.gender.group), ]

results_d.lg.gender.group[c(1:12,17:21,24), ] <- purrr::modify_if(results_d.lg.gender.group[c(1:12,17:21,24), ], ~is.numeric(.), ~round(., 2))
results_d.lg.gender.group[c(22:23), ] <- purrr::modify_if(results_d.lg.gender.group[c(22:23), ], ~is.numeric(.), ~round(., 0))

results_d.lg.gender.group[13,] <- p_judge(results_d.lg.gender.group[13,])
results_d.lg.gender.group[14,] <- p_judge(results_d.lg.gender.group[14,])
results_d.lg.gender.group[15,] <- p_judge(results_d.lg.gender.group[15,])
results_d.lg.gender.group[16,] <- p_judge(results_d.lg.gender.group[16,])

results_d.lg.gender.group <- results_d.lg.gender.group[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_d.lg.gender.group, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_d.lg.gender.group))), pageLength = nrow(results_d.lg.gender.group)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group <- meta_results_list_cohort$meta_d.lg.gender.group
meta_d.lg.gender.group_age <- meta_d.lg.gender.group$age
summary(meta_d.lg.gender.group_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group <- meta_results_list_cohort$meta_d.lg.gender.group
meta_d.lg.gender.group_gender <- meta_d.lg.gender.group$gender
summary(meta_d.lg.gender.group_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group <- meta_results_list_cohort$meta_d.lg.gender.group
meta_d.lg.gender.group_age.group <- meta_d.lg.gender.group$age.group
summary(meta_d.lg.gender.group_age.group)
```
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Age trajectories of driving risk-taking propensity for each sample and meta-analysis. The solid black line indicates the average trajectory weighted by the sample size. The colored lines represent the trajectory for individual samples"}
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_d.lg.gender.group.inter <- results_table_list_cohort$results_d.lg.gender.group.inter
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2", "d.c.age:gender_Std.Error2", "d.c.age:age.group_Std.Error2", "gender:age.group_Std.Error2", "d.c.age:gender:age.group_Std.Error2")
results_d.lg.gender.group.inter <- results_d.lg.gender.group.inter[!(row.names(results_d.lg.gender.group.inter) %in% remove),]
rownames(results_d.lg.gender.group.inter)[rownames(results_d.lg.gender.group.inter)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_d.lg.gender.group.inter)[rownames(results_d.lg.gender.group.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_d.lg.gender.group.inter)[rownames(results_d.lg.gender.group.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_d.lg.gender.group.inter)[rownames(results_d.lg.gender.group.inter)=="age.group_Std.Error1"] <- "age.group_Std.Error"
rownames(results_d.lg.gender.group.inter)[rownames(results_d.lg.gender.group.inter)=="d.c.age:gender_group_Std.Error1"] <- "d.c.age:gender_Std.Error"
rownames(results_d.lg.gender.group.inter)[rownames(results_d.lg.gender.group.inter)=="d.c.age:age.group_Std.Error1"] <- "d.c.age:age.group_Std.Error"
rownames(results_d.lg.gender.group.inter)[rownames(results_d.lg.gender.group.inter)=="gender:age.group_Std.Error1"] <- "gender:age.group_Std.Error"
rownames(results_d.lg.gender.group.inter)[rownames(results_d.lg.gender.group.inter)=="d.c.age:gender:age.group_Std.Error1"] <- "d.c.age:gender:age.group_Std.Error"
results_d.lg.gender.group.inter <- results_d.lg.gender.group.inter[, colSums(is.na(results_d.lg.gender.group.inter)) != nrow(results_d.lg.gender.group.inter)]
results_d.lg.gender.group.inter <- results_d.lg.gender.group.inter[complete.cases(results_d.lg.gender.group.inter), ]

results_d.lg.gender.group.inter[c(1:24,33:37,40), ] <- purrr::modify_if(results_d.lg.gender.group.inter[c(1:24,33:37,40), ], ~is.numeric(.), ~round(., 2))
results_d.lg.gender.group.inter[c(38:39), ] <- purrr::modify_if(results_d.lg.gender.group.inter[c(38:39), ], ~is.numeric(.), ~round(., 0))

results_d.lg.gender.group.inter[25,] <- p_judge(results_d.lg.gender.group.inter[25,])
results_d.lg.gender.group.inter[26,] <- p_judge(results_d.lg.gender.group.inter[26,])
results_d.lg.gender.group.inter[27,] <- p_judge(results_d.lg.gender.group.inter[27,])
results_d.lg.gender.group.inter[28,] <- p_judge(results_d.lg.gender.group.inter[28,])
results_d.lg.gender.group.inter[29,] <- p_judge(results_d.lg.gender.group.inter[29,])
results_d.lg.gender.group.inter[30,] <- p_judge(results_d.lg.gender.group.inter[30,])
results_d.lg.gender.group.inter[31,] <- p_judge(results_d.lg.gender.group.inter[31,])
results_d.lg.gender.group.inter[32,] <- p_judge(results_d.lg.gender.group.inter[32,])

results_d.lg.gender.group.inter <- results_d.lg.gender.group.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value","d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "d.c.age:age.group_Estimate", "d.c.age:age.group_Std.Error", "d.c.age:age.group_t.value", "d.c.age:age.group_p.value", "gender:age.group_Estimate", "gender:age.group_Std.Error", "gender:age.group_t.value", "gender:age.group_p.value", "d.c.age:gender:age.group_Estimate", "d.c.age:gender:age.group_Std.Error", "d.c.age:gender:age.group_t.value", "d.c.age:gender:age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_d.lg.gender.group.inter, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_d.lg.gender.group.inter))), pageLength = nrow(results_d.lg.gender.group.inter)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group.inter <- meta_results_list_cohort$meta_d.lg.gender.group.inter
meta_d.lg.gender.group.inter_age <- meta_d.lg.gender.group.inter$age
summary(meta_d.lg.gender.group.inter_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group.inter <- meta_results_list_cohort$meta_d.lg.gender.group.inter
meta_d.lg.gender.group.inter_gender <- meta_d.lg.gender.group.inter$gender
summary(meta_d.lg.gender.group.inter_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group.inter <- meta_results_list_cohort$meta_d.lg.gender.group.inter
meta_d.lg.gender.group.inter_age.group <- meta_d.lg.gender.group.inter$age.group
summary(meta_d.lg.gender.group.inter_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group.inter <- meta_results_list_cohort$meta_d.lg.gender.group.inter
meta_d.lg.gender.group.inter_age_gender <- meta_d.lg.gender.group.inter$age_gender
summary(meta_d.lg.gender.group.inter_age_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group.inter <- meta_results_list_cohort$meta_d.lg.gender.group.inter
meta_d.lg.gender.group.inter_age_age.group <- meta_d.lg.gender.group.inter$age_age.group
summary(meta_d.lg.gender.group.inter_age_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group.inter <- meta_results_list_cohort$meta_d.lg.gender.group.inter
meta_d.lg.gender.group.inter_gender_age.group <- meta_d.lg.gender.group.inter$gender_age.group
summary(meta_d.lg.gender.group.inter_gender_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_d.lg.gender.group.inter <- meta_results_list_cohort$meta_d.lg.gender.group.inter
meta_d.lg.gender.group.inter_age_gender_age.group <- meta_d.lg.gender.group.inter$age_gender_age.group
summary(meta_d.lg.gender.group.inter_age_gender_age.group)
```
...
#### Recreational risk-taking{.tabset}
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_r.lg.gender.group <- results_table_list_cohort$results_r.lg.gender.group
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2")
results_r.lg.gender.group <- results_r.lg.gender.group[!(row.names(results_r.lg.gender.group) %in% remove),]
rownames(results_r.lg.gender.group)[rownames(results_r.lg.gender.group)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_r.lg.gender.group)[rownames(results_r.lg.gender.group)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_r.lg.gender.group)[rownames(results_r.lg.gender.group)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_r.lg.gender.group)[rownames(results_r.lg.gender.group)=="age.group_Std.Error1"] <- "age.group_Std.Error"
results_r.lg.gender.group <- results_r.lg.gender.group[, colSums(is.na(results_r.lg.gender.group)) != nrow(results_r.lg.gender.group)]
results_r.lg.gender.group <- results_r.lg.gender.group[complete.cases(results_r.lg.gender.group), ]

results_r.lg.gender.group[c(1:12,17:21,24), ] <- purrr::modify_if(results_r.lg.gender.group[c(1:12,17:21,24), ], ~is.numeric(.), ~round(., 2))
results_r.lg.gender.group[c(22:23), ] <- purrr::modify_if(results_r.lg.gender.group[c(22:23), ], ~is.numeric(.), ~round(., 0))

results_r.lg.gender.group[13,] <- p_judge(results_r.lg.gender.group[13,])
results_r.lg.gender.group[14,] <- p_judge(results_r.lg.gender.group[14,])
results_r.lg.gender.group[15,] <- p_judge(results_r.lg.gender.group[15,])
results_r.lg.gender.group[16,] <- p_judge(results_r.lg.gender.group[16,])

results_r.lg.gender.group <- results_r.lg.gender.group[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_r.lg.gender.group, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_r.lg.gender.group))), pageLength = nrow(results_r.lg.gender.group)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group <- meta_results_list_cohort$meta_r.lg.gender.group
meta_r.lg.gender.group_age <- meta_r.lg.gender.group$age
summary(meta_r.lg.gender.group_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group <- meta_results_list_cohort$meta_r.lg.gender.group
meta_r.lg.gender.group_gender <- meta_r.lg.gender.group$gender
summary(meta_r.lg.gender.group_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group <- meta_results_list_cohort$meta_r.lg.gender.group
meta_r.lg.gender.group_age.group <- meta_r.lg.gender.group$age.group
summary(meta_r.lg.gender.group_age.group)
```
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Age trajectories of recreational risk-taking propensity for each sample and meta-analysis. The solid black line indicates the average trajectory weighted by the sample size. The colored lines represent the trajectory for individual samples"}
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_r.lg.gender.group.inter <- results_table_list_cohort$results_r.lg.gender.group.inter
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2", "d.c.age:gender_Std.Error2", "d.c.age:age.group_Std.Error2", "gender:age.group_Std.Error2", "d.c.age:gender:age.group_Std.Error2")
results_r.lg.gender.group.inter <- results_r.lg.gender.group.inter[!(row.names(results_r.lg.gender.group.inter) %in% remove),]
rownames(results_r.lg.gender.group.inter)[rownames(results_r.lg.gender.group.inter)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_r.lg.gender.group.inter)[rownames(results_r.lg.gender.group.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_r.lg.gender.group.inter)[rownames(results_r.lg.gender.group.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_r.lg.gender.group.inter)[rownames(results_r.lg.gender.group.inter)=="age.group_Std.Error1"] <- "age.group_Std.Error"
rownames(results_r.lg.gender.group.inter)[rownames(results_r.lg.gender.group.inter)=="d.c.age:gender_group_Std.Error1"] <- "d.c.age:gender_Std.Error"
rownames(results_r.lg.gender.group.inter)[rownames(results_r.lg.gender.group.inter)=="d.c.age:age.group_Std.Error1"] <- "d.c.age:age.group_Std.Error"
rownames(results_r.lg.gender.group.inter)[rownames(results_r.lg.gender.group.inter)=="gender:age.group_Std.Error1"] <- "gender:age.group_Std.Error"
rownames(results_r.lg.gender.group.inter)[rownames(results_r.lg.gender.group.inter)=="d.c.age:gender:age.group_Std.Error1"] <- "d.c.age:gender:age.group_Std.Error"
results_r.lg.gender.group.inter <- results_r.lg.gender.group.inter[, colSums(is.na(results_r.lg.gender.group.inter)) != nrow(results_r.lg.gender.group.inter)]
results_r.lg.gender.group.inter <- results_r.lg.gender.group.inter[complete.cases(results_r.lg.gender.group.inter), ]

results_r.lg.gender.group.inter[c(1:24,33:37,40), ] <- purrr::modify_if(results_r.lg.gender.group.inter[c(1:24,33:37,40), ], ~is.numeric(.), ~round(., 2))
results_r.lg.gender.group.inter[c(38:39), ] <- purrr::modify_if(results_r.lg.gender.group.inter[c(38:39), ], ~is.numeric(.), ~round(., 0))

results_r.lg.gender.group.inter[25,] <- p_judge(results_r.lg.gender.group.inter[25,])
results_r.lg.gender.group.inter[26,] <- p_judge(results_r.lg.gender.group.inter[26,])
results_r.lg.gender.group.inter[27,] <- p_judge(results_r.lg.gender.group.inter[27,])
results_r.lg.gender.group.inter[28,] <- p_judge(results_r.lg.gender.group.inter[28,])
results_r.lg.gender.group.inter[29,] <- p_judge(results_r.lg.gender.group.inter[29,])
results_r.lg.gender.group.inter[30,] <- p_judge(results_r.lg.gender.group.inter[30,])
results_r.lg.gender.group.inter[31,] <- p_judge(results_r.lg.gender.group.inter[31,])
results_r.lg.gender.group.inter[32,] <- p_judge(results_r.lg.gender.group.inter[32,])

results_r.lg.gender.group.inter <- results_r.lg.gender.group.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value","d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "d.c.age:age.group_Estimate", "d.c.age:age.group_Std.Error", "d.c.age:age.group_t.value", "d.c.age:age.group_p.value", "gender:age.group_Estimate", "gender:age.group_Std.Error", "gender:age.group_t.value", "gender:age.group_p.value", "d.c.age:gender:age.group_Estimate", "d.c.age:gender:age.group_Std.Error", "d.c.age:gender:age.group_t.value", "d.c.age:gender:age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_r.lg.gender.group.inter, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_r.lg.gender.group.inter))), pageLength = nrow(results_r.lg.gender.group.inter)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group.inter <- meta_results_list_cohort$meta_r.lg.gender.group.inter
meta_r.lg.gender.group.inter_age <- meta_r.lg.gender.group.inter$age
summary(meta_r.lg.gender.group.inter_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group.inter <- meta_results_list_cohort$meta_r.lg.gender.group.inter
meta_r.lg.gender.group.inter_gender <- meta_r.lg.gender.group.inter$gender
summary(meta_r.lg.gender.group.inter_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group.inter <- meta_results_list_cohort$meta_r.lg.gender.group.inter
meta_r.lg.gender.group.inter_age.group <- meta_r.lg.gender.group.inter$age.group
summary(meta_r.lg.gender.group.inter_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group.inter <- meta_results_list_cohort$meta_r.lg.gender.group.inter
meta_r.lg.gender.group.inter_age_gender <- meta_r.lg.gender.group.inter$age_gender
summary(meta_r.lg.gender.group.inter_age_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group.inter <- meta_results_list_cohort$meta_r.lg.gender.group.inter
meta_r.lg.gender.group.inter_age_age.group <- meta_r.lg.gender.group.inter$age_age.group
summary(meta_r.lg.gender.group.inter_age_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group.inter <- meta_results_list_cohort$meta_r.lg.gender.group.inter
meta_r.lg.gender.group.inter_gender_age.group <- meta_r.lg.gender.group.inter$gender_age.group
summary(meta_r.lg.gender.group.inter_gender_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_r.lg.gender.group.inter <- meta_results_list_cohort$meta_r.lg.gender.group.inter
meta_r.lg.gender.group.inter_age_gender_age.group <- meta_r.lg.gender.group.inter$age_gender_age.group
summary(meta_r.lg.gender.group.inter_age_gender_age.group)
```
...
#### Occupational risk-taking{.tabset}
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_o.lg.gender.group <- results_table_list_cohort$results_o.lg.gender.group
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2")
results_o.lg.gender.group <- results_o.lg.gender.group[!(row.names(results_o.lg.gender.group) %in% remove),]
rownames(results_o.lg.gender.group)[rownames(results_o.lg.gender.group)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_o.lg.gender.group)[rownames(results_o.lg.gender.group)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_o.lg.gender.group)[rownames(results_o.lg.gender.group)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_o.lg.gender.group)[rownames(results_o.lg.gender.group)=="age.group_Std.Error1"] <- "age.group_Std.Error"
results_o.lg.gender.group <- results_o.lg.gender.group[, colSums(is.na(results_o.lg.gender.group)) != nrow(results_o.lg.gender.group)]
results_o.lg.gender.group <- results_o.lg.gender.group[complete.cases(results_o.lg.gender.group), ]

results_o.lg.gender.group[c(1:12,17:21,24), ] <- purrr::modify_if(results_o.lg.gender.group[c(1:12,17:21,24), ], ~is.numeric(.), ~round(., 2))
results_o.lg.gender.group[c(22:23), ] <- purrr::modify_if(results_o.lg.gender.group[c(22:23), ], ~is.numeric(.), ~round(., 0))

results_o.lg.gender.group[13,] <- p_judge(results_o.lg.gender.group[13,])
results_o.lg.gender.group[14,] <- p_judge(results_o.lg.gender.group[14,])
results_o.lg.gender.group[15,] <- p_judge(results_o.lg.gender.group[15,])
results_o.lg.gender.group[16,] <- p_judge(results_o.lg.gender.group[16,])

results_o.lg.gender.group <- results_o.lg.gender.group[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_o.lg.gender.group, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_o.lg.gender.group))), pageLength = nrow(results_o.lg.gender.group)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group <- meta_results_list_cohort$meta_o.lg.gender.group
meta_o.lg.gender.group_age <- meta_o.lg.gender.group$age
summary(meta_o.lg.gender.group_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group <- meta_results_list_cohort$meta_o.lg.gender.group
meta_o.lg.gender.group_gender <- meta_o.lg.gender.group$gender
summary(meta_o.lg.gender.group_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group <- meta_results_list_cohort$meta_o.lg.gender.group
meta_o.lg.gender.group_age.group <- meta_o.lg.gender.group$age.group
summary(meta_o.lg.gender.group_age.group)
```
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Age trajectories of occupational risk-taking propensity for each sample and meta-analysis. The solid black line indicates the average trajectory weighted by the sample size. The colored lines represent the trajectory for individual samples"}
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_o.lg.gender.group.inter <- results_table_list_cohort$results_o.lg.gender.group.inter
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2", "d.c.age:gender_Std.Error2", "d.c.age:age.group_Std.Error2", "gender:age.group_Std.Error2", "d.c.age:gender:age.group_Std.Error2")
results_o.lg.gender.group.inter <- results_o.lg.gender.group.inter[!(row.names(results_o.lg.gender.group.inter) %in% remove),]
rownames(results_o.lg.gender.group.inter)[rownames(results_o.lg.gender.group.inter)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_o.lg.gender.group.inter)[rownames(results_o.lg.gender.group.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_o.lg.gender.group.inter)[rownames(results_o.lg.gender.group.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_o.lg.gender.group.inter)[rownames(results_o.lg.gender.group.inter)=="age.group_Std.Error1"] <- "age.group_Std.Error"
rownames(results_o.lg.gender.group.inter)[rownames(results_o.lg.gender.group.inter)=="d.c.age:gender_group_Std.Error1"] <- "d.c.age:gender_Std.Error"
rownames(results_o.lg.gender.group.inter)[rownames(results_o.lg.gender.group.inter)=="d.c.age:age.group_Std.Error1"] <- "d.c.age:age.group_Std.Error"
rownames(results_o.lg.gender.group.inter)[rownames(results_o.lg.gender.group.inter)=="gender:age.group_Std.Error1"] <- "gender:age.group_Std.Error"
rownames(results_o.lg.gender.group.inter)[rownames(results_o.lg.gender.group.inter)=="d.c.age:gender:age.group_Std.Error1"] <- "d.c.age:gender:age.group_Std.Error"
results_o.lg.gender.group.inter <- results_o.lg.gender.group.inter[, colSums(is.na(results_o.lg.gender.group.inter)) != nrow(results_o.lg.gender.group.inter)]
results_o.lg.gender.group.inter <- results_o.lg.gender.group.inter[complete.cases(results_o.lg.gender.group.inter), ]

results_o.lg.gender.group.inter[c(1:24,33:37,40), ] <- purrr::modify_if(results_o.lg.gender.group.inter[c(1:24,33:37,40), ], ~is.numeric(.), ~round(., 2))
results_o.lg.gender.group.inter[c(38:39), ] <- purrr::modify_if(results_o.lg.gender.group.inter[c(38:39), ], ~is.numeric(.), ~round(., 0))

results_o.lg.gender.group.inter[25,] <- p_judge(results_o.lg.gender.group.inter[25,])
results_o.lg.gender.group.inter[26,] <- p_judge(results_o.lg.gender.group.inter[26,])
results_o.lg.gender.group.inter[27,] <- p_judge(results_o.lg.gender.group.inter[27,])
results_o.lg.gender.group.inter[28,] <- p_judge(results_o.lg.gender.group.inter[28,])
results_o.lg.gender.group.inter[29,] <- p_judge(results_o.lg.gender.group.inter[29,])
results_o.lg.gender.group.inter[30,] <- p_judge(results_o.lg.gender.group.inter[30,])
results_o.lg.gender.group.inter[31,] <- p_judge(results_o.lg.gender.group.inter[31,])
results_o.lg.gender.group.inter[32,] <- p_judge(results_o.lg.gender.group.inter[32,])

results_o.lg.gender.group.inter <- results_o.lg.gender.group.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value","d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "d.c.age:age.group_Estimate", "d.c.age:age.group_Std.Error", "d.c.age:age.group_t.value", "d.c.age:age.group_p.value", "gender:age.group_Estimate", "gender:age.group_Std.Error", "gender:age.group_t.value", "gender:age.group_p.value", "d.c.age:gender:age.group_Estimate", "d.c.age:gender:age.group_Std.Error", "d.c.age:gender:age.group_t.value", "d.c.age:gender:age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_o.lg.gender.group.inter, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_o.lg.gender.group.inter))), pageLength = nrow(results_o.lg.gender.group.inter)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group.inter <- meta_results_list_cohort$meta_o.lg.gender.group.inter
meta_o.lg.gender.group.inter_age <- meta_o.lg.gender.group.inter$age
summary(meta_o.lg.gender.group.inter_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group.inter <- meta_results_list_cohort$meta_o.lg.gender.group.inter
meta_o.lg.gender.group.inter_gender <- meta_o.lg.gender.group.inter$gender
summary(meta_o.lg.gender.group.inter_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group.inter <- meta_results_list_cohort$meta_o.lg.gender.group.inter
meta_o.lg.gender.group.inter_age.group <- meta_o.lg.gender.group.inter$age.group
summary(meta_o.lg.gender.group.inter_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group.inter <- meta_results_list_cohort$meta_o.lg.gender.group.inter
meta_o.lg.gender.group.inter_age_gender <- meta_o.lg.gender.group.inter$age_gender
summary(meta_o.lg.gender.group.inter_age_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group.inter <- meta_results_list_cohort$meta_o.lg.gender.group.inter
meta_o.lg.gender.group.inter_age_age.group <- meta_o.lg.gender.group.inter$age_age.group
summary(meta_o.lg.gender.group.inter_age_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group.inter <- meta_results_list_cohort$meta_o.lg.gender.group.inter
meta_o.lg.gender.group.inter_gender_age.group <- meta_o.lg.gender.group.inter$gender_age.group
summary(meta_o.lg.gender.group.inter_gender_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_o.lg.gender.group.inter <- meta_results_list_cohort$meta_o.lg.gender.group.inter
meta_o.lg.gender.group.inter_age_gender_age.group <- meta_o.lg.gender.group.inter$age_gender_age.group
summary(meta_o.lg.gender.group.inter_age_gender_age.group)
```
<div style="margin-bottom:20px;">
</div>
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_h.lg.gender.group <- results_table_list_cohort$results_h.lg.gender.group
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2")
results_h.lg.gender.group <- results_h.lg.gender.group[!(row.names(results_h.lg.gender.group) %in% remove),]
rownames(results_h.lg.gender.group)[rownames(results_h.lg.gender.group)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_h.lg.gender.group)[rownames(results_h.lg.gender.group)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_h.lg.gender.group)[rownames(results_h.lg.gender.group)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_h.lg.gender.group)[rownames(results_h.lg.gender.group)=="age.group_Std.Error1"] <- "age.group_Std.Error"
results_h.lg.gender.group <- results_h.lg.gender.group[, colSums(is.na(results_h.lg.gender.group)) != nrow(results_h.lg.gender.group)]
results_h.lg.gender.group <- results_h.lg.gender.group[complete.cases(results_h.lg.gender.group), ]

results_h.lg.gender.group[c(1:12,17:21,24), ] <- purrr::modify_if(results_h.lg.gender.group[c(1:12,17:21,24), ], ~is.numeric(.), ~round(., 2))
results_h.lg.gender.group[c(22:23), ] <- purrr::modify_if(results_h.lg.gender.group[c(22:23), ], ~is.numeric(.), ~round(., 0))

results_h.lg.gender.group[13,] <- p_judge(results_h.lg.gender.group[13,])
results_h.lg.gender.group[14,] <- p_judge(results_h.lg.gender.group[14,])
results_h.lg.gender.group[15,] <- p_judge(results_h.lg.gender.group[15,])
results_h.lg.gender.group[16,] <- p_judge(results_h.lg.gender.group[16,])

results_h.lg.gender.group <- results_h.lg.gender.group[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_h.lg.gender.group, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_h.lg.gender.group))), pageLength = nrow(results_h.lg.gender.group)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group <- meta_results_list_cohort$meta_h.lg.gender.group
meta_h.lg.gender.group_age <- meta_h.lg.gender.group$age
summary(meta_h.lg.gender.group_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group <- meta_results_list_cohort$meta_h.lg.gender.group
meta_h.lg.gender.group_gender <- meta_h.lg.gender.group$gender
summary(meta_h.lg.gender.group_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group <- meta_results_list_cohort$meta_h.lg.gender.group
meta_h.lg.gender.group_age.group <- meta_h.lg.gender.group$age.group
summary(meta_h.lg.gender.group_age.group)
```
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Age trajectories of health risk-taking propensity for each sample and meta-analysis. The solid black line indicates the average trajectory weighted by the sample size. The colored lines represent the trajectory for individual samples"}
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_h.lg.gender.group.inter <- results_table_list_cohort$results_h.lg.gender.group.inter
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2", "d.c.age:gender_Std.Error2", "d.c.age:age.group_Std.Error2", "gender:age.group_Std.Error2", "d.c.age:gender:age.group_Std.Error2")
results_h.lg.gender.group.inter <- results_h.lg.gender.group.inter[!(row.names(results_h.lg.gender.group.inter) %in% remove),]
rownames(results_h.lg.gender.group.inter)[rownames(results_h.lg.gender.group.inter)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_h.lg.gender.group.inter)[rownames(results_h.lg.gender.group.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_h.lg.gender.group.inter)[rownames(results_h.lg.gender.group.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_h.lg.gender.group.inter)[rownames(results_h.lg.gender.group.inter)=="age.group_Std.Error1"] <- "age.group_Std.Error"
rownames(results_h.lg.gender.group.inter)[rownames(results_h.lg.gender.group.inter)=="d.c.age:gender_group_Std.Error1"] <- "d.c.age:gender_Std.Error"
rownames(results_h.lg.gender.group.inter)[rownames(results_h.lg.gender.group.inter)=="d.c.age:age.group_Std.Error1"] <- "d.c.age:age.group_Std.Error"
rownames(results_h.lg.gender.group.inter)[rownames(results_h.lg.gender.group.inter)=="gender:age.group_Std.Error1"] <- "gender:age.group_Std.Error"
rownames(results_h.lg.gender.group.inter)[rownames(results_h.lg.gender.group.inter)=="d.c.age:gender:age.group_Std.Error1"] <- "d.c.age:gender:age.group_Std.Error"
results_h.lg.gender.group.inter <- results_h.lg.gender.group.inter[, colSums(is.na(results_h.lg.gender.group.inter)) != nrow(results_h.lg.gender.group.inter)]
results_h.lg.gender.group.inter <- results_h.lg.gender.group.inter[complete.cases(results_h.lg.gender.group.inter), ]

results_h.lg.gender.group.inter[c(1:24,33:37,40), ] <- purrr::modify_if(results_h.lg.gender.group.inter[c(1:24,33:37,40), ], ~is.numeric(.), ~round(., 2))
results_h.lg.gender.group.inter[c(38:39), ] <- purrr::modify_if(results_h.lg.gender.group.inter[c(38:39), ], ~is.numeric(.), ~round(., 0))

results_h.lg.gender.group.inter[25,] <- p_judge(results_h.lg.gender.group.inter[25,])
results_h.lg.gender.group.inter[26,] <- p_judge(results_h.lg.gender.group.inter[26,])
results_h.lg.gender.group.inter[27,] <- p_judge(results_h.lg.gender.group.inter[27,])
results_h.lg.gender.group.inter[28,] <- p_judge(results_h.lg.gender.group.inter[28,])
results_h.lg.gender.group.inter[29,] <- p_judge(results_h.lg.gender.group.inter[29,])
results_h.lg.gender.group.inter[30,] <- p_judge(results_h.lg.gender.group.inter[30,])
results_h.lg.gender.group.inter[31,] <- p_judge(results_h.lg.gender.group.inter[31,])
results_h.lg.gender.group.inter[32,] <- p_judge(results_h.lg.gender.group.inter[32,])

results_h.lg.gender.group.inter <- results_h.lg.gender.group.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value","d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "d.c.age:age.group_Estimate", "d.c.age:age.group_Std.Error", "d.c.age:age.group_t.value", "d.c.age:age.group_p.value", "gender:age.group_Estimate", "gender:age.group_Std.Error", "gender:age.group_t.value", "gender:age.group_p.value", "d.c.age:gender:age.group_Estimate", "d.c.age:gender:age.group_Std.Error", "d.c.age:gender:age.group_t.value", "d.c.age:gender:age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),]

datatable(results_h.lg.gender.group.inter, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_h.lg.gender.group.inter))), pageLength = nrow(results_h.lg.gender.group.inter)))
```
...
*Age effect results*
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group.inter <- meta_results_list_cohort$meta_h.lg.gender.group.inter
meta_h.lg.gender.group.inter_age <- meta_h.lg.gender.group.inter$age
summary(meta_h.lg.gender.group.inter_age)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group.inter <- meta_results_list_cohort$meta_h.lg.gender.group.inter
meta_h.lg.gender.group.inter_gender <- meta_h.lg.gender.group.inter$gender
summary(meta_h.lg.gender.group.inter_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group.inter <- meta_results_list_cohort$meta_h.lg.gender.group.inter
meta_h.lg.gender.group.inter_age.group <- meta_h.lg.gender.group.inter$age.group
summary(meta_h.lg.gender.group.inter_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group.inter <- meta_results_list_cohort$meta_h.lg.gender.group.inter
meta_h.lg.gender.group.inter_age_gender <- meta_h.lg.gender.group.inter$age_gender
summary(meta_h.lg.gender.group.inter_age_gender)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group.inter <- meta_results_list_cohort$meta_h.lg.gender.group.inter
meta_h.lg.gender.group.inter_age_age.group <- meta_h.lg.gender.group.inter$age_age.group
summary(meta_h.lg.gender.group.inter_age_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group.inter <- meta_results_list_cohort$meta_h.lg.gender.group.inter
meta_h.lg.gender.group.inter_gender_age.group <- meta_h.lg.gender.group.inter$gender_age.group
summary(meta_h.lg.gender.group.inter_gender_age.group)
```
...
```{r, echo = F, warning = FALSE, message = FALSE}
meta_h.lg.gender.group.inter <- meta_results_list_cohort$meta_h.lg.gender.group.inter
meta_h.lg.gender.group.inter_age_gender_age.group <- meta_h.lg.gender.group.inter$age_gender_age.group
summary(meta_h.lg.gender.group.inter_age_gender_age.group)
```
...
#### Social risk-taking{.tabset}

<div style="margin-bottom:10px;">
</div>

This section refers to the section "Cohort effects" in the main manuscript. It reports results for 2 additional multilevel models of social domain.
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_s.lg.gender.group <- results_table_list_cohort$results_s.lg.gender.group
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2")
results_s.lg.gender.group <- results_s.lg.gender.group[!(row.names(results_s.lg.gender.group) %in% remove),]
rownames(results_s.lg.gender.group)[rownames(results_s.lg.gender.group)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_s.lg.gender.group)[rownames(results_s.lg.gender.group)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_s.lg.gender.group)[rownames(results_s.lg.gender.group)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_s.lg.gender.group)[rownames(results_s.lg.gender.group)=="age.group_Std.Error1"] <- "age.group_Std.Error"
results_s.lg.gender.group <- results_s.lg.gender.group[, colSums(is.na(results_s.lg.gender.group)) != nrow(results_s.lg.gender.group), drop = FALSE]
results_s.lg.gender.group <- results_s.lg.gender.group[complete.cases(results_s.lg.gender.group), 1, drop = FALSE]

results_s.lg.gender.group[c(1:12,17:21,24), ] <- purrr::modify_if(results_s.lg.gender.group[c(1:12,17:21,24), ], ~is.numeric(.), ~round(., 2))
results_s.lg.gender.group[c(22:23), ] <- purrr::modify_if(results_s.lg.gender.group[c(22:23), ], ~is.numeric(.), ~round(., 0))

results_s.lg.gender.group[13,] <- p_judge(results_s.lg.gender.group[13,])
results_s.lg.gender.group[14,] <- p_judge(results_s.lg.gender.group[14,])
results_s.lg.gender.group[15,] <- p_judge(results_s.lg.gender.group[15,])
results_s.lg.gender.group[16,] <- p_judge(results_s.lg.gender.group[16,])

results_s.lg.gender.group <- results_s.lg.gender.group[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"), 1, drop = FALSE]

datatable(results_s.lg.gender.group, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_s.lg.gender.group))), pageLength = nrow(results_s.lg.gender.group)))
```
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Age trajectories of social risk-taking propensity for each sample and meta-analysis. The solid black line indicates the average trajectory weighted by the sample size. The colored lines represent the trajectory for individual samples"}
b=7
c=1
y_name <- paste0(domains[b],"_z_mean")
  y_number <- paste0(domains[b],"_z_n")
...
<br>
```{r, echo = F, warning = FALSE, message = FALSE}
results_s.lg.gender.group.inter <- results_table_list_cohort$results_s.lg.gender.group.inter
remove <- c("Intercept_Std.Error2", "d.c.age_Std.Error2", "gender_Std.Error2", "age.group_Std.Error2", "d.c.age:gender_Std.Error2", "d.c.age:age.group_Std.Error2", "gender:age.group_Std.Error2", "d.c.age:gender:age.group_Std.Error2")
results_s.lg.gender.group.inter <- results_s.lg.gender.group.inter[!(row.names(results_s.lg.gender.group.inter) %in% remove),]
rownames(results_s.lg.gender.group.inter)[rownames(results_s.lg.gender.group.inter)=="Intercept_Std.Error1"] <- "Intercept_Std.Error"
rownames(results_s.lg.gender.group.inter)[rownames(results_s.lg.gender.group.inter)=="d.c.age_Std.Error1"] <- "d.c.age_Std.Error"
rownames(results_s.lg.gender.group.inter)[rownames(results_s.lg.gender.group.inter)=="gender_Std.Error1"] <- "gender_Std.Error"
rownames(results_s.lg.gender.group.inter)[rownames(results_s.lg.gender.group.inter)=="age.group_Std.Error1"] <- "age.group_Std.Error"
rownames(results_s.lg.gender.group.inter)[rownames(results_s.lg.gender.group.inter)=="d.c.age:gender_group_Std.Error1"] <- "d.c.age:gender_Std.Error"
rownames(results_s.lg.gender.group.inter)[rownames(results_s.lg.gender.group.inter)=="d.c.age:age.group_Std.Error1"] <- "d.c.age:age.group_Std.Error"
rownames(results_s.lg.gender.group.inter)[rownames(results_s.lg.gender.group.inter)=="gender:age.group_Std.Error1"] <- "gender:age.group_Std.Error"
rownames(results_s.lg.gender.group.inter)[rownames(results_s.lg.gender.group.inter)=="d.c.age:gender:age.group_Std.Error1"] <- "d.c.age:gender:age.group_Std.Error"
results_s.lg.gender.group.inter <- results_s.lg.gender.group.inter[, colSums(is.na(results_s.lg.gender.group.inter)) != nrow(results_s.lg.gender.group.inter), drop = FALSE]
results_s.lg.gender.group.inter <- results_s.lg.gender.group.inter[complete.cases(results_s.lg.gender.group.inter), 1, drop = FALSE]

results_s.lg.gender.group.inter[c(1:24,33:37,40), ] <- purrr::modify_if(results_s.lg.gender.group.inter[c(1:24,33:37,40), ], ~is.numeric(.), ~round(., 2))
results_s.lg.gender.group.inter[c(38:39), ] <- purrr::modify_if(results_s.lg.gender.group.inter[c(38:39), ], ~is.numeric(.), ~round(., 0))

results_s.lg.gender.group.inter[25,] <- p_judge(results_s.lg.gender.group.inter[25,])
results_s.lg.gender.group.inter[26,] <- p_judge(results_s.lg.gender.group.inter[26,])
results_s.lg.gender.group.inter[27,] <- p_judge(results_s.lg.gender.group.inter[27,])
results_s.lg.gender.group.inter[28,] <- p_judge(results_s.lg.gender.group.inter[28,])
results_s.lg.gender.group.inter[29,] <- p_judge(results_s.lg.gender.group.inter[29,])
results_s.lg.gender.group.inter[30,] <- p_judge(results_s.lg.gender.group.inter[30,])
results_s.lg.gender.group.inter[31,] <- p_judge(results_s.lg.gender.group.inter[31,])
results_s.lg.gender.group.inter[32,] <- p_judge(results_s.lg.gender.group.inter[32,])

results_s.lg.gender.group.inter <- results_s.lg.gender.group.inter[c("Intercept_Estimate", "Intercept_Std.Error", "Intercept_t.value", "Intercept_p.value", "d.c.age_Estimate", "d.c.age_Std.Error", "d.c.age_t.value", "d.c.age_p.value", "gender_Estimate","gender_Std.Error", "gender_t.value", "gender_p.value", "age.group_Estimate", "age.group_Std.Error", "age.group_t.value", "age.group_p.value","d.c.age:gender_Estimate", "d.c.age:gender_Std.Error", "d.c.age:gender_t.value", "d.c.age:gender_p.value", "d.c.age:age.group_Estimate", "d.c.age:age.group_Std.Error", "d.c.age:age.group_t.value", "d.c.age:age.group_p.value", "gender:age.group_Estimate", "gender:age.group_Std.Error", "gender:age.group_t.value", "gender:age.group_p.value", "d.c.age:gender:age.group_Estimate", "d.c.age:gender:age.group_Std.Error", "d.c.age:gender:age.group_t.value", "d.c.age:gender:age.group_p.value", "Within-group-variance", "Between-group-variance", "Random-slope-variance", "Slope-Intercept-covariance", "Slope-Intercept-correlation", "N.group", "N.obs", "LL"),1, drop = FALSE]

datatable(results_s.lg.gender.group.inter, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(results_s.lg.gender.group.inter))), pageLength = nrow(results_s.lg.gender.group.inter)))
```
...
### 8.2. Models comparison
```{r, echo = F, warning = FALSE, message = FALSE}
Total.Number <- c(9, 20, 3, 3, 3, 3, 1)
Lg.gender.group <- c(8, 17, 2, 2, 3, 3, 1)
Lg.gender.group.inter <- c(1, 3, 1, 1, 0, 0, 0)


Table4 <- data.frame(Total.Number, Lg.gender.group, Lg.gender.group.inter)
rownames(Table4) <- c("General","Financial", "Driving", "Recreational", "Occupational", "Health", "Social")
...
sketch = htmltools::withTags(table(
  ...
  thead(
    tr(
      th(rowspan = 2, ''),
      th(rowspan = 2, 'Total number'),
      th(colspan = 2, 'Number of samples with best-fitting'),
    ),
    tr(
      lapply(c("Linear with gender and age group", "Linear with gender and age group interaction"), th)
    )
  )
))

datatable(Table4 , container = sketch, options = list(columnDefs = list(list(className = 'dt-center', targets = 1:ncol(Table4))), pageLength = nrow(Table4)))
```

<div style="margin-bottom:40px;">
</div>

## 9. Identifying variance components {.tabset}
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Variance decomposition of age effect", fig.align="center"}
...
<br>
<br>
```{r, echo = F, warning = FALSE, message = FALSE, fig.cap = "Figure: Variance decomposition of gender effect", fig.align="center"}
