...
require(devtools)
install_version("sjstats", version = "0.17.4", repos = "http://cran.us.r-project.org")  #the version of sjstats must be 0.17.4, else can not run successfully,because icc function in "sjstats 0.17.4" package
      ...
      for (c in 1:length(models)) {
        ...
        if(c==1){
          ...
          intdummy_model_list[[model_dummy_name]] <- dataset %>% 
            ...
            mutate(icc = unlist(lapply(.$models, sjstats::icc)))
    ...
    for (c in 1:length(models)) {
      ...
      if(c==1){
        if (!is.null(model_list[[model_name]])){
          results_list[[results_name]] <- list(coef = coef(summary(model_list[[model_name]])),
...
### function 8: transform results_list into dataframe and save them into previous results_table_list ###
...
### save results
setwd("../data/process_z_data/results_data")
saveRDS(results_table_list, file ="results_table_list.Rds")
saveRDS(plot_data_results_list, file ="plot_data_results_list.Rds")
