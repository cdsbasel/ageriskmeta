...
SOEP_process <- pldata  %>%
  ...
  select(ID, SYEAR, BIRTH, Gender, General, Driving, Financial, Recreational, Occupational, Health, Social) %>%
  filter(BIRTH>0) %>%
  mutate(age=SYEAR-BIRTH) %>%
  filter(!is.na(age)) %>%
  filter(!is.na(Gender)) %>%
  select(ID, SYEAR, Gender, age, General, Driving, Financial, Recreational, Occupational, Health, Social)  %>%
