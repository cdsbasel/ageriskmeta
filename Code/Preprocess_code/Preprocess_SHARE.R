...
setwd("../rawdata/SHARE_data_new")

#WAVE 1: there is not risk aversion

#WAVE 2：2006/2007
dems2 <- read.spss("sharew2_rel7-1-0_cv_r.sav", use.value.label=FALSE, to.data.frame=TRUE) %>%
  select(mergeid, gender, country, age_int)
risk2 <- read.spss("sharew2_rel7-1-0_as.sav", use.value.label=FALSE, to.data.frame=TRUE) %>%
  select(mergeid, country, as068_)

data2 <- Reduce(function(x, y) merge(x, y, all=TRUE), list(dems2, risk2))  %>%
  rename(ID=mergeid, Gender=gender, age=age_int, risk_aversion=as068_) %>%
  mutate(SYEAR=2007)
...
risk4 <- read.spss("sharew4_rel7-1-0_ex.sav", use.value.label=FALSE, to.data.frame=TRUE) %>%
  select(mergeid, country, ex110_)

data4 <- Reduce(function(x, y) merge(x, y, all=TRUE), list(dems4, risk4))  %>%
  rename(ID=mergeid, Gender=gender, age=age_int, risk_aversion=ex110_) %>%
  mutate(SYEAR=2011)
...
SHARE_preprocess <- SHARE_data %>%
  filter(!is.na(risk_aversion)) %>%
  rename(Financial_raw = risk_aversion) %>%
  mutate(Financial_reverse = 5-Financial_raw) %>% #reverse
  mutate(Financial = 10*(Financial_reverse-1)/(4-1)) %>% #record to 10 scale
  select(ID, SYEAR,  Gender, age, country, Financial_raw, Financial_reverse, Financial) %>% 
  filter(!is.na (age)) %>%
  filter(!is.na (Gender)) %>%
