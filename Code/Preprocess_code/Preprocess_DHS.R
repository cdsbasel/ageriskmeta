...
#6. spaar6: 
#7. beschryf
#8. teg9: trust people, reverse(only in panel 1993, 1995, 1997, 1998, 1999, 2000, 2001, 2002, not in panel 1994, 1996, 2003-2019) #seems should not include it
...
path="~/Desktop/analysis/meta_analysis_risk_normalized_5/rawdata/DHS_data"
setwd(path)
...
filesdems=matrix(data=NA,nrow=29,ncol=1)
filesdems[1]=dir(path,pattern = "wrk1993", full.names = TRUE, ignore.case = TRUE) # demographics are given in wrk SPSS in 1993
filesdems[2:29]=dir(path,pattern = "hh", full.names = TRUE, ignore.case = TRUE) # demographics are given in hhi SPSS from 1994 onwards
...
#DHS_data=Reduce(function(x, y) merge(x, y, all=TRUE), list(dems_total, psy_total)) %>%
#  unique() 

#NOTE: when merge two dataframe, "merge" and "Reduce(function(x, y) merge(x, y, all=TRUE))" both can work
# but when use merge, please add "all=TRUE" command, then can get complete dataset, if not include "all=TRUE" command, the row including NA will be removed.
...
DHS_preprocess <- DHS_data %>%
  dplyr::rename(BIRTH=gebjaar, Gender=geslacht) %>%
  dplyr::select(ID, BIRTH, Gender, SYEAR, Financial_raw, Financial) %>%
  dplyr::filter(BIRTH > 0) %>%
  dplyr::filter(SYEAR > 0) %>%
  dplyr::filter(!is.na(Gender)) %>%
  mutate(age = SYEAR-BIRTH) %>%
  dplyr::select(ID, SYEAR, Gender, age, Financial_raw, Financial) %>%
  dplyr::filter(!is.na (Financial)) %>%
...
#DHS_process_z <- readRDS(file = "DHS_process_z.Rds")
