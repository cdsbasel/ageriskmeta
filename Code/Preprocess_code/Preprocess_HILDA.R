...
for(i in 1:length(waves)){
  ...
  if(i < 5) {
    input <- input %>%
      select(xwaveid, paste(waveletter, "hhhqivw", sep=""), paste(waveletter, "hgage1", sep=""), paste(waveletter, "hgsex1", sep=""), paste(waveletter, "firisk", sep="")) %>%
      rename(ID=xwaveid, inter_date=paste(waveletter, "hhhqivw", sep=""), age=paste(waveletter, "hgage1", sep=""), Gender=paste(waveletter, "hgsex1", sep=""), risk=paste(waveletter, "firisk", sep="")) %>%
      mutate(inter_date = strptime(inter_date, "%d/%m/%Y")) %>%
      mutate(SYEAR=SYEAR[i]) 
      input$risk[which(input$risk==5)] = NA
  }else{
    input <- input %>%
      select(xwaveid, paste(waveletter, "hhhqivw", sep=""), paste(waveletter, "hgage1", sep=""), paste(waveletter, "hgsex1", sep=""),paste(waveletter, "firisk", sep=""), paste(waveletter, "firiska", sep="")) %>%
      rename(ID=xwaveid, inter_date=paste(waveletter, "hhhqivw", sep=""), age=paste(waveletter, "hgage1", sep=""), Gender=paste(waveletter, "hgsex1", sep=""), risk=paste(waveletter, "firisk", sep=""), risk_assumed=paste(waveletter, "firiska", sep="")) %>%
      mutate(inter_date = strptime(inter_date, "%d/%m/%Y")) %>%
      mutate(SYEAR = SYEAR[i]) 
...
HILDA_preprocess <- Reduce(function(x, y) merge(x, y, all=TRUE), list(wave1, wave2, wave3, wave4, wave6, wave8, wave10, wave11, wave12, wave13, wave14, wave15, wave16, wave17, wave18, wave19)) %>%
  select(ID, SYEAR, age, Gender, risk) %>%
  filter(!is.na(Gender)) %>%
  filter(!is.na(age)) %>%
  filter(!is.na(risk)) %>%
