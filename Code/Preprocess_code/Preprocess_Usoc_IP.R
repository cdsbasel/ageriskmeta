...
setwd("../rawdata/USoc_data_IP")

#wave1(2008)
wave1 <- read.spss("a_indresp_ip.sav", use.value.label=FALSE, to.data.frame=TRUE) %>%
...
#wave6(2013)
wave6 <- read.spss("f_indresp_ip.sav", use.value.label=FALSE, to.data.frame=TRUE) %>%
...
#wave7(2014)
wave7 <- read.spss("g_indresp_ip.sav", use.value.label=FALSE, to.data.frame=TRUE) %>%
