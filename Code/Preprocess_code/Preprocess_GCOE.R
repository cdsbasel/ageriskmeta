...
setwd("~/Desktop/analysis/meta_analysis_risk_normalized_5/rawdata/GCOE_data")

###Japan GCOE Preprocess
###2003-2010###
# Japan_2003_q22; because the question of 2003 is not same as other year, so should not combine with other years' items
#syear=2003
#Japan2003 <- read_dta("Japan/2003Data_JAPAN_prefid.dta") %>%
#  select(seqno,panelid,q22,fq1,fq3) #q22:NA=99; fq1:1…2; fq3:2…6
#Japan2003$SYEAR <- syear
#Japan2003$ID=paste(Japan2003$seqno,Japan2003$panelid,sep="_")
#Japan2003 <- Japan2003 %>%
#  rename(General= q22, Gender=fq1, age_cat=fq3) %>% #in 2003, age was represented with bins(10 years)
#  mutate(age=NA)
...
age_process_long <- Japan_process_wide %>%
  gather(key = SYEAR, value=age, 4:10)  #spread：from long to wide; gather:from wide to long
...
#GCOE_USA_process_z <- readRDS(file = "GCOE_USA_process_z.Rds")
