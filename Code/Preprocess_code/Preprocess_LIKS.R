...
dems2013 <- read.spss("2013/hh1a.sav", use.value.label=FALSE, to.data.frame=TRUE) %>%
  select(hhid, pid, h102, h103_m, h103_y, h103a)
...
LIKS_preprocess <- LIKS_data %>%
  select(ID, Gender, age, SYEAR, General) %>%
  filter(!is.na(Gender)) %>%
