...
setwd("../rawdata/PHF_data")

# wave 1
data_p1 <- read_dta("PHF_p_wave1_v3_0.dta")  %>%
  filter(pid == impid) %>%
  select(hhid, impid, pid, persid, ra0300, dpe9040)

data_h1 <- read_dta("PHF_h_wave1_v3_0.dta") %>%
  select(hhid, impid, intjahr, zi103, hd1800, dhd2800)
...
PHF_preprocess <- rbind(data1, data2, data3) %>%
  rename(ID=persid, age=ra0300, Gender=dpe9040, General=zi103, Financial_household=hd1800, Financial_raw=dhd2800) %>% 
  mutate(BIRTH=intjahr-age) %>% 
  select(ID, SYEAR, Gender, age, General, Financial_raw) %>% # should not include social, include financial 
